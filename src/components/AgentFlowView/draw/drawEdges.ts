/**
 * Edge drawing with tapered bezier curves.
 */

import type { Agent, ToolCallNode, Edge } from '../simulation/types'
import { BEAM, ANIM, MIN_VISIBLE_OPACITY } from '../lib/canvas-constants'
...
function resolveEdgeTarget(
  ...
  if (toAgent && toAgent.opacity >= MIN_VISIBLE_OPACITY) return toAgent
  const toTool = toolCalls.get(edge.to)
  if (toTool && toTool.opacity >= MIN_VISIBLE_OPACITY) return toTool
  return null
}
